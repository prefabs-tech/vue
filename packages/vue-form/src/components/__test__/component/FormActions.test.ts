...
describe("FormActions", () => {
  it("renders default Submit and Cancel buttons", () => {
    const wrapper = mount(FormActions, {
      global: {
        stubs: {
          ButtonElement: {
            props: ["label", "type", "severity", "variant", "disabled"],
            template: `<button :type="type" :data-severity="severity" :data-variant="variant" :disabled="disabled">{{ label }}</button>`,
          },
        },
      },
    });
  ...
  it("applies default submitLabel and cancelLabel", () => {
    const wrapper = mount(FormActions, {
      global: {
        stubs: {
          ButtonElement: {
            props: ["label", "type", "severity", "variant"],
            template: `<button :type="type" :data-severity="severity" :data-variant="variant">{{ label }}</button>`,
          },
        },
      },
    });
  ...
  it("applies default button types and styles", () => {
    const wrapper = mount(FormActions, {
      global: {
        stubs: {
          ButtonElement: {
    ...
    // Cancel button should have severity="secondary" and variant="outlined"
    expect(buttons[1].attributes("data-severity")).toBe("secondary");
    expect(buttons[1].attributes("data-variant")).toBe("outlined");
  });

  it("accepts custom submitLabel and cancelLabel props", () => {
    const wrapper = mount(FormActions, {
      props: {
        submitLabel: "Save",
        cancelLabel: "Go Back",
  ...
  it("applies alignment prop as data attribute", () => {
    const wrapper = mount(FormActions, {
      props: {
        alignment: "left",
      },
    ...
    const container = wrapper.find(".form-actions");
    expect(container.attributes("data-alignment")).toBe("left");
  });

  it("applies flowDirection prop as data attribute", () => {
    const wrapper = mount(FormActions, {
      props: {
    ...
    const container = wrapper.find(".form-actions");
    expect(container.attributes("data-direction")).toBe("vertical");
  });

  it("applies reverse prop as data attribute", () => {
    const wrapper = mount(FormActions, {
      props: {
        reverse: true,
      },
    ...
    const container = wrapper.find(".form-actions");
    expect(container.attributes("data-reverse")).toBe("true");
  });
});
